...
class View {
	...
	public function __construct( $routeObj = '' ) {
		...
		//编译路径
		$this->pathArr['cache'] = $this->routeObj->controllerPath . '/' .
			$this->routeObj->groupName . 
			'/cache/' . 
			$this->routeObj->className . '/';

		//文件路径
		$this->pathArr['view'] = $this->routeObj->controllerPath . '/' .
		 	$this->routeObj->groupName . 
		 	'/view/' . 
			$this->routeObj->className . '/';
	}
	...
	private function _compresFile($templateFile = null) {
		$tmpfile = !empty($templateFile) ? $templateFile : $this->routeObj->actionName;
		//编译文件路径
